...
export default function LocalInvoiceDetails({ id }: { id: string }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f7fb] px-4 py-8 text-slate-950"><div className="mx-auto max-w-2xl">
      ...
      <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber-700">Local-only invoice</p><h1 className="mt-2 text-3xl font-black">{invoice.invoiceNumber}</h1><p className="mt-6 text-lg font-black">{invoice.customerName}</p><p className="mt-1 text-slate-500">{invoice.customerPhone || "No phone number"}</p><div className="my-6 border-t border-slate-200" /><p className="font-bold">{invoice.description}</p><p className="mt-2 text-slate-500">{invoice.quantity} × {currency(invoice.price)}</p><div className="mt-6 flex justify-between border-t border-slate-200 pt-4"><span className="font-bold">Total</span><span className="text-2xl font-black">{currency(invoice.total)}</span></div><p className="mt-4 text-sm font-bold uppercase text-slate-500">Status: {invoice.status}</p></section>
