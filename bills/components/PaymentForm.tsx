...
export default function PaymentForm({ invoice, onSaved }: Props) {
  ...
  if (invoice.status === "paid") return <p className="rounded-xl bg-emerald-50 p-4 text-sm font-bold text-emerald-800">Paid in full. Payment recorded manually.</p>;

  return <>
    <button type="button" onClick={() => setOpen(true)} className="min-h-14 w-full rounded-2xl bg-emerald-50 px-5 py-4 font-black text-emerald-700 print:hidden">Record payment</button>
