...
export default function InvoicePaymentPanel({ invoiceNumber, total, outstandingAmount = total, dueDays }: Props) {
  ...
  return (
    <section className="mt-6 rounded-3xl border border-blue-100 bg-blue-50 p-5 print:hidden">
      <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-700">Payment details</p><h2 className="mt-1 text-xl font-black">Pay by UPI</h2></div><button type="button" onClick={() => setEditing((value) => !value)} className="text-sm font-bold text-blue-700 underline">{editing ? "Close" : "Edit settings"}</button></div>
      ...
      <div className="mt-5 flex flex-col items-center gap-4 sm:flex-row sm:items-start">{payload && qrCode ? <Image src={qrCode} alt="UPI payment QR code" width={220} height={220} unoptimized className="rounded-xl bg-white p-2" /> : <div className="flex h-[220px] w-[220px] items-center justify-center rounded-xl bg-white p-5 text-center text-sm font-semibold text-slate-500">Add your UPI ID to show a payment QR code.</div>}<div className="text-sm text-slate-700"><p className="font-bold">{settings.businessName}</p><p className="mt-1">UPI ID: {settings.upiId || "Not configured"}</p>{payload && <a href={payload} className="mt-4 inline-block rounded-xl bg-blue-600 px-4 py-3 font-bold text-white">Tap to Pay</a>}<p className="mt-3 text-xs text-slate-600">{settings.upiId ? "Scan with any UPI app." : "Use Edit settings to configure payments."}</p><p className="mt-3 text-xs text-slate-500">Terms: {paymentTerms(dueDays)}</p></div></div>
    </section>
  );
}
