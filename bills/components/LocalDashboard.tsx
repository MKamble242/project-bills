      ...
      <div className="mx-auto min-h-screen max-w-6xl px-4 py-4 sm:px-6 lg:px-8">
        ...
        {loading ? <p className="mt-8 font-bold">Loading local invoices...</p> : <>
          ...
          <section className="mt-8"><div className="mb-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Your workspace</p><h2 className="mt-1 text-2xl font-black tracking-tight">Recent records</h2></div>
            {invoices.length === 0 ? <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-2xl">₹</div><h3 className="mt-4 text-lg font-black">No records yet</h3><p className="mx-auto mt-2 max-w-sm text-sm text-slate-500">Add your first Hisab Kitab record and it will stay on this device.</p><Link href="/invoices/new" className="mt-6 inline-flex min-h-12 items-center rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white">Add first record</Link></div> : <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">{invoices.slice(0, 10).map((invoice, index) => {
